...
var instruments := [
    {"name":"Πιάνο","symbol":"🎹"},{"name":"Κιθάρα","symbol":"🎸"},{"name":"Βιολί","symbol":"🎻"},
    {"name":"Τύμπανο","symbol":"🥁"},{"name":"Τρομπέτα","symbol":"🎺"},{"name":"Σαξόφωνο","symbol":"🎷"},
    {"name":"Φλάουτο","symbol":"🪈"},{"name":"Ακορντεόν","symbol":"🪗"},{"name":"Μαράκες","symbol":"🪇"},
    {"name":"Μικρόφωνο","symbol":"🎤"}
]
...
func _build() -> void:
    ...
    var title := Label.new()
    title.text = "Κουίζ Μουσικών Οργάνων"
...
func _new_round() -> void:
    ...
    feedback_label.text = ""
    next_button.visible = false
    correct_index = randi_range(0, instruments.size() - 1)
    target_label.text = "Βρες: " + instruments[correct_index]["name"]
    ...
    while choices.size() < 4:
        var candidate := randi_range(0, instruments.size() - 1)
    ...
    for index in choices:
        ...
        button.text = instruments[index]["symbol"]
...
func _answer(chosen: int) -> void:
    ...
    if chosen == correct_index:
        answered = true
        feedback_label.text = "Μπράβο! Σωστό όργανο! ⭐"
        stars += 1
        stars_label.text = "⭐ %d" % stars
...
func _speak_current() -> void:
    ...
    if voices.size() > 0:
        ...
        DisplayServer.tts_speak(instruments[correct_index]["name"], voices[0])
