...
var foods := [
    {"name":"Ψωμί","symbol":"🍞"},{"name":"Γάλα","symbol":"🥛"},{"name":"Τυρί","symbol":"🧀"},
    {"name":"Γιαούρτι","symbol":"🥣"},{"name":"Αυγό","symbol":"🥚"},{"name":"Ρύζι","symbol":"🍚"},
    {"name":"Μακαρόνια","symbol":"🍝"},{"name":"Πατάτες τηγανητές","symbol":"🍟"},{"name":"Πίτσα","symbol":"🍕"},
    {"name":"Σάντουιτς","symbol":"🥪"},{"name":"Σούπα","symbol":"🍲"},{"name":"Σαλάτα","symbol":"🥗"},
    {"name":"Κοτόπουλο","symbol":"🍗"},{"name":"Ψάρι","symbol":"🐟"},{"name":"Μέλι","symbol":"🍯"},
    {"name":"Μπισκότο","symbol":"🍪"},{"name":"Κέικ","symbol":"🍰"},{"name":"Παγωτό","symbol":"🍦"},
    {"name":"Σοκολάτα","symbol":"🍫"},{"name":"Δημητριακά","symbol":"🥣"}
]
...
func _new_round() -> void:
    ...
    feedback_label.text = ""
    next_button.visible = false
    correct_index = randi_range(0, foods.size() - 1)
    target_label.text = "Βρες: " + foods[correct_index]["name"]
    ...
    while choices.size() < 4:
        var candidate := randi_range(0, foods.size() - 1)
    ...
    for value in choices:
        ...
        button.text = foods[value]["symbol"]
...
func _answer(chosen: int) -> void:
    ...
    if chosen == correct_index:
        answered = true
        feedback_label.text = "Μπράβο! Σωστό τρόφιμο! ⭐"
        stars += 1
        stars_label.text = "⭐ %d" % stars
...
func _speak_current() -> void:
    ...
    if voices.size() > 0:
        ...
        DisplayServer.tts_speak(foods[correct_index]["name"], voices[0])
