...
var vegetables := [
    {"name":"Καρότο","symbol":"🥕"},{"name":"Ντομάτα","symbol":"🍅"},
    {"name":"Πατάτα","symbol":"🥔"},{"name":"Κρεμμύδι","symbol":"🧅"},
    {"name":"Σκόρδο","symbol":"🧄"},{"name":"Αγγούρι","symbol":"🥒"},
    {"name":"Πιπεριά","symbol":"🫑"},{"name":"Μελιτζάνα","symbol":"🍆"},
    {"name":"Μπρόκολο","symbol":"🥦"},{"name":"Κουνουπίδι","symbol":"🥦"},
    {"name":"Μαρούλι","symbol":"🥬"},{"name":"Λάχανο","symbol":"🥬"},
    {"name":"Καλαμπόκι","symbol":"🌽"},{"name":"Μπιζέλι","symbol":"🟢"},
    {"name":"Φασολάκι","symbol":"🫛"},{"name":"Κολοκύθα","symbol":"🎃"},
    {"name":"Κολοκυθάκι","symbol":"🥒"},{"name":"Παντζάρι","symbol":"🔴"},
    {"name":"Ραπανάκι","symbol":"🔴"},{"name":"Σπανάκι","symbol":"🥬"}
]
...
func _new_round() -> void:
    ...
    feedback_label.text = ""
    next_button.visible = false
    correct_index = randi_range(0, vegetables.size() - 1)
    target_label.text = "Βρες: " + vegetables[correct_index]["name"]
    ...
    while choices.size() < 4:
        var candidate := randi_range(0, vegetables.size() - 1)
    ...
    for value in choices:
        ...
        button.text = vegetables[value]["symbol"]
...
func _answer(chosen: int) -> void:
    ...
    if chosen == correct_index:
        answered = true
        feedback_label.text = "Μπράβο! Σωστό λαχανικό! ⭐"
        stars += 1
        stars_label.text = "⭐ %d" % stars
...
func _speak_current() -> void:
    ...
    if voices.size() > 0:
        ...
        DisplayServer.tts_speak(vegetables[correct_index]["name"], voices[0])
