...
var vehicles := [
    {"name":"Αυτοκίνητο","symbol":"🚗"},{"name":"Λεωφορείο","symbol":"🚌"},
    {"name":"Τρένο","symbol":"🚆"},{"name":"Αεροπλάνο","symbol":"✈️"},
    {"name":"Ελικόπτερο","symbol":"🚁"},{"name":"Πλοίο","symbol":"🚢"},
    {"name":"Βάρκα","symbol":"⛵"},{"name":"Ποδήλατο","symbol":"🚲"},
    {"name":"Μοτοσικλέτα","symbol":"🏍️"},{"name":"Ασθενοφόρο","symbol":"🚑"}
]
...
func _new_round() -> void:
    ...
    feedback_label.text = ""
    next_button.visible = false
    correct_index = randi_range(0, vehicles.size() - 1)
    target_label.text = "Βρες: " + vehicles[correct_index]["name"]
    ...
    while choices.size() < 4:
        var candidate := randi_range(0, vehicles.size() - 1)
    ...
    for index in choices:
        ...
        button.text = vehicles[index]["symbol"]
...
func _answer(chosen: int) -> void:
    ...
    if chosen == correct_index:
        answered = true
        feedback_label.text = "Μπράβο! Σωστό όχημα! ⭐"
        stars += 1
        stars_label.text = "⭐ %d" % stars
...
func _speak_current() -> void:
    ...
    if voices.size() > 0:
        ...
        DisplayServer.tts_speak(vehicles[correct_index]["name"], voices[0])
