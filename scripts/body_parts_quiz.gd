...
var parts := [
    {"name":"Κεφάλι","symbol":"🙂"},{"name":"Μάτια","symbol":"👀"},{"name":"Αυτιά","symbol":"👂"},
    {"name":"Μύτη","symbol":"👃"},{"name":"Στόμα","symbol":"👄"},{"name":"Χέρι","symbol":"✋"},
    {"name":"Δάχτυλα","symbol":"🖐️"},{"name":"Πόδι","symbol":"🦵"},{"name":"Πατούσα","symbol":"🦶"},{"name":"Καρδιά","symbol":"❤️"}
]
...
func _new_round() -> void:
    ...
    feedback_label.text = ""
    next_button.visible = false
    correct_index = randi_range(0, parts.size() - 1)
    target_label.text = "Βρες: " + parts[correct_index]["name"]
    ...
    while choices.size() < 4:
        var candidate := randi_range(0, parts.size() - 1)
    ...
    for index in choices:
        ...
        button.text = parts[index]["symbol"]
...
func _speak_current() -> void:
    ...
    if voices.size() > 0:
        ...
        DisplayServer.tts_speak(parts[correct_index]["name"], voices[0])
