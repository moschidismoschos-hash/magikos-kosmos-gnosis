...
var fruits := [
    {"name":"Μήλο","symbol":"🍎"},{"name":"Μπανάνα","symbol":"🍌"},{"name":"Πορτοκάλι","symbol":"🍊"},
    {"name":"Αχλάδι","symbol":"🍐"},{"name":"Φράουλα","symbol":"🍓"},{"name":"Καρπούζι","symbol":"🍉"},
    {"name":"Σταφύλι","symbol":"🍇"},{"name":"Κεράσι","symbol":"🍒"},{"name":"Ροδάκινο","symbol":"🍑"},
    {"name":"Ανανάς","symbol":"🍍"},{"name":"Λεμόνι","symbol":"🍋"},{"name":"Μάνγκο","symbol":"🥭"},
    {"name":"Ακτινίδιο","symbol":"🥝"},{"name":"Πεπόνι","symbol":"🍈"},{"name":"Μανταρίνι","symbol":"🍊"},
    {"name":"Γκρέιπφρουτ","symbol":"🍊"},{"name":"Βερίκοκο","symbol":"🍑"},{"name":"Δαμάσκηνο","symbol":"🟣"},
    {"name":"Σύκο","symbol":"🟤"},{"name":"Ρόδι","symbol":"🔴"},{"name":"Καρύδα","symbol":"🥥"},
    {"name":"Αβοκάντο","symbol":"🥑"},{"name":"Παπάγια","symbol":"🧡"},{"name":"Βατόμουρο","symbol":"🫐"},
    {"name":"Μύρτιλο","symbol":"🫐"},{"name":"Σμέουρο","symbol":"🔴"},{"name":"Λωτός","symbol":"🟠"}
]
...
func _new_round() -> void:
    ...
    feedback_label.text = ""
    next_button.visible = false
    correct_index = randi_range(0, fruits.size() - 1)
    target_label.text = "Βρες: " + fruits[correct_index]["name"]
    ...
    while choices.size() < 4:
        var candidate := randi_range(0, fruits.size() - 1)
    ...
    for value in choices:
        ...
        button.text = fruits[value]["symbol"]
...
func _answer(chosen: int) -> void:
    ...
    if chosen == correct_index:
        answered = true
        feedback_label.text = "Μπράβο! Σωστό φρούτο! ⭐"
        stars += 1
        stars_label.text = "⭐ %d" % stars
...
func _speak_current() -> void:
    ...
    if voices.size() > 0:
        ...
        DisplayServer.tts_speak(fruits[correct_index]["name"], voices[0])
