...
func _build() -> void:
    ...
    title_label.text = "Γράψε τον αριθμό"
    ...
    var hear := Button.new()
    hear.text = "🔊 Άκουσε"
    hear.custom_minimum_size = Vector2(170, 52)
    ...
    clear.custom_minimum_size = Vector2(180, 52)
    clear.pressed.connect(_clear_canvas)
    controls.add_child(clear)

    var canvas_panel := PanelContainer.new()
    canvas_panel.custom_minimum_size = Vector2(0, 405)
    ...
    guide_label.add_theme_font_size_override("font_size", 260)
...
func _show_number() -> void:
    guide_label.text = str(current_number)
    title_label.text = "Γράψε τον αριθμό " + str(current_number)
    _clear_canvas()
...
func _speak_current() -> void:
    ...
    if voices.size() > 0:
        ...
        DisplayServer.tts_speak(str(current_number), voices[0])
...
func _complete() -> void:
    if strokes.size() == 0:
        title_label.text = "Σχεδίασε πρώτα τον αριθμό."
        return

    stars += 1
    stars_label.text = "⭐ %d" % stars
    _save_value("stars", stars)
    title_label.text = "Μπράβο! Έγραψες το " + str(current_number) + " ⭐"
