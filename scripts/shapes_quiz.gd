...
var shapes := [
    {"name":"Κύκλος","symbol":"●"},{"name":"Τετράγωνο","symbol":"■"},
    {"name":"Τρίγωνο","symbol":"▲"},{"name":"Ορθογώνιο","symbol":"▭"},
    {"name":"Αστέρι","symbol":"★"},{"name":"Καρδιά","symbol":"♥"},
    {"name":"Οβάλ","symbol":"⬭"},{"name":"Ρόμβος","symbol":"◆"}
]
...
func _new_round() -> void:
    ...
    feedback_label.text = ""
    next_button.visible = false
    correct_index = randi_range(0, shapes.size() - 1)
    target_label.text = "Βρες το: " + shapes[correct_index]["name"]
    ...
    while choices.size() < 4:
        var candidate := randi_range(0, shapes.size() - 1)
    ...
    for index in choices:
        ...
        button.text = shapes[index]["symbol"]
...
func _answer(chosen: int) -> void:
    ...
    if chosen == correct_index:
        answered = true
        feedback_label.text = "Μπράβο! Σωστό σχήμα! ⭐"
        stars += 1
        stars_label.text = "⭐ %d" % stars
...
func _speak_current() -> void:
    ...
    if voices.size() > 0:
        ...
        DisplayServer.tts_speak(shapes[correct_index]["name"], voices[0])
