...
var clothes := [
    {"name":"Μπλούζα","symbol":"👕"},{"name":"Παντελόνι","symbol":"👖"},
    {"name":"Φούστα","symbol":"👗"},{"name":"Φόρεμα","symbol":"👗"},
    {"name":"Πουκάμισο","symbol":"👔"},{"name":"Μπουφάν","symbol":"🧥"},
    {"name":"Ζακέτα","symbol":"🧥"},{"name":"Παλτό","symbol":"🧥"},
    {"name":"Κάλτσες","symbol":"🧦"},{"name":"Παπούτσια","symbol":"👞"},
    {"name":"Αθλητικά παπούτσια","symbol":"👟"},{"name":"Πέδιλα","symbol":"👡"},
    {"name":"Μπότες","symbol":"🥾"},{"name":"Παντόφλες","symbol":"🥿"},
    {"name":"Καπέλο","symbol":"🎩"},{"name":"Σκουφί","symbol":"🧢"},
    {"name":"Καπέλο ηλίου","symbol":"👒"},{"name":"Κασκόλ","symbol":"🧣"},
    {"name":"Γάντια","symbol":"🧤"},{"name":"Ζώνη","symbol":"🟫"},
    {"name":"Γραβάτα","symbol":"👔"},{"name":"Πιτζάμα","symbol":"🛌"},
    {"name":"Μαγιό","symbol":"🩱"},{"name":"Σορτς","symbol":"🩳"},
    {"name":"Αδιάβροχο","symbol":"🧥"}
]
...
func _new_round() -> void:
    ...
    feedback_label.text = ""
    next_button.visible = false
    correct_index = randi_range(0, clothes.size() - 1)
    target_label.text = "Βρες: " + clothes[correct_index]["name"]
    ...
    while choices.size() < 4:
        var candidate := randi_range(0, clothes.size() - 1)
    ...
    for value in choices:
        ...
        button.text = clothes[value]["symbol"]
...
func _answer(chosen: int) -> void:
    ...
    if chosen == correct_index:
        answered = true
        feedback_label.text = "Μπράβο! Σωστό ρούχο! ⭐"
        stars += 1
        stars_label.text = "⭐ %d" % stars
...
func _speak_current() -> void:
    ...
    if voices.size() > 0:
        ...
        DisplayServer.tts_speak(clothes[correct_index]["name"], voices[0])
